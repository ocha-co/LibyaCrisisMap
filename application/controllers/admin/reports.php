	...
	*/
	function index($page = 1)
	{
		...
		if (!empty($_GET['status']))
		{
			...
			{
				$filter = 'incident_verified = 0 AND incident_active = 1';
			}
			elseif (strtolower($status) == 'x')
			{
			    $filter = 'incident_verified = 2';
			}
			else
			{
				$status = "0";
				...
				elseif ($post->action == 'x')
				  {
					...
					$verify = new Verify_Model();
					if ($update->loaded == true) {
echo					  $update->incident_verified;
					  if ($update->incident_active == '1')
					    {
					      $update->incident_verified = '2';
					      $update->save();
					      ...
					      $t++;
					    } // End Unverify button edit 07.04.2011 @ line 218 /controller/admin/reports.php
					}
				      }
				    $form_action = "UNVERIFIED (".$t.")";
				  }	  
	...
	*/
	function edit( $id = false, $saved = false )
	{
		...
		if ($_POST)
		{
			...
			$post->add_rules('incident_photo', 'upload::valid', 'upload::type[gif,jpg,png,pdf]', 'upload::size[2M]');
	...
	*/
	...
	function download()
	{
		...
		if ($_POST)
		{
			...
			$post->add_rules('modify_from_date','date_mmddyyyy');
			$post->add_rules('modify_to_date','date_mmddyyyy');
			...
			if (!empty($_POST['modify_from_date']) || !empty($_POST['modify_to_date']))
			{
				// Valid FROM Date?
				if (empty($_POST['modify_from_date']) || (strtotime($_POST['modify_from_date']) > strtotime("today"))) {
					$post->add_error('modify_from_date','range');
				}

				// Valid TO date?
				if (empty($_POST['modify_to_date']) || (strtotime($_POST['modify_to_date']) > strtotime("today"))) {
					$post->add_error('modify_to_date','range');
				}

				// TO Date not greater than FROM Date?
				if (strtotime($_POST['modify_from_date']) > strtotime($_POST['modify_to_date'])) {
					$post->add_error('to_date','range_greater');
				}
			}
			...
			if ($post->validate())
			{
				...
				{
					$filter .= " AND ( incident_date >= '" . date("Y-m-d H:i:s",strtotime($post->from_date)) . "' AND incident_date <= '" . date("Y-m-d H:i:s",strtotime($post->to_date.' 23:59:59')) . "' ) ";
				}
				if (!empty($post->modify_from_date) && !empty($post->modify_to_date))
				{
					$filter .= " AND ( incident_datemodify >= '" . date("Y-m-d H:i:s",strtotime($post->modify_from_date)) . "' AND incident_datemodify <= '" . date("Y-m-d H:i:s",strtotime($post->modify_to_date.' 23:59:59')) . "' OR ".
						"(incident_datemodify is null and incident_dateadd >= '" . date("Y-m-d H:i:s",strtotime($post->modify_from_date)) . "' AND incident_dateadd <= '" . date("Y-m-d H:i:s",strtotime($post->modify_to_date.' 23:59:59')) . "')) ";
				}
				
				// Column Titles
				$report_csv = "#,INCIDENT TITLE,INCIDENT DATE";
				$catcol = 3;
				$casualties = $loadCategories = $split_cats = false;
				...
				if($casualties)
					$report_csv .= ",DEATH,INJURED,DISPLACED,WOMEN,CHILDREN";
				$report_csv .= ",APPROVED,VERIFIED";
				$report_csv .= "\n";
				// Retrieve reports
				$incidents = Incident_Model::getIncidents($filter, true);
				if($casualties)
					$formData =  Incident_Model::getFormData($filter);
				foreach ($incidents as $incident)
				{
					$emtpyCols   = '';
					$addRow      = '';
								...
								foreach($incident->incident_category as $category)
								{
									...
									if($split_cats){
										if ($i == 0)
											...
											$addRow .="\n".$emtpyCols.'"'.$this->_csv_text($category->category->category_title).'"';
										$i++;
									}else 
										$report_csv .= $this->_csv_text($category->category->category_title).', ';
								}
							...
							break;
 							case 6:
								...
								foreach($incident->media as $media)
                                                        ...
                                                        break;
							case 8:
								$report_csv .= ',"'.(isset($formData[$incident->id]['2']) ? $formData[$incident->id]['2'] : '').'"';
								$report_csv .= ',"'.(isset($formData[$incident->id]['3']) ? $formData[$incident->id]['3'] : '').'"';
								$report_csv .= ',"'.(isset($formData[$incident->id]['4']) ? $formData[$incident->id]['4'] : '').'"';
								$report_csv .= ',"'.(isset($formData[$incident->id]['5']) ? $formData[$incident->id]['5'] : '').'"';
								$report_csv .= ',"'.(isset($formData[$incident->id]['6']) ? $formData[$incident->id]['6'] : '').'"';
						}
					}
		...
		$this->template->content->errors = $errors;
		$this->template->content->form_error = $form_error;
		$this->template->content->categories = Category_Model::getAllCategories();
	...
	*/
	...
	private function _get_categories()
	{
		$categories = ORM::factory('category')
			...
			->where("parent_id = 0 and category_title not like '%Deprecated%'")
	...
	{
	  return htmlspecialchars_decode(str_replace('"', "'", $text));//stripslashes(htmlspecialchars($text))
	}
}
