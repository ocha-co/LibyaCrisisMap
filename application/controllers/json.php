...
class Json_Controller extends Template_Controller
{
	...
	function index()
	{
		...
		foreach ($markers as $marker)
		{
			...
			$json_item .= "\"name\":\"" . date("n/j/Y", strtotime($marker->incident_date)). " - ".str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href='".url::base()."reports/view/".$marker->id."'>".$encoded_title)."</a>") . "\",";
	...
	public function cluster()
	{
		...
		foreach ($singles as $single)
		{
			...
			$json_item .= "\"type\":\"Feature\",";
			$json_item .= "\"properties\": {";
			$json_item .= "\"name\":\"" . date("n/j/Y",strtotime($single['incident_date']))." - ". str_replace(chr(10), ' ', str_replace(chr(13), ' ', "<a href=" . url::base() . "reports/view/" . $single['id'] . "/>".str_replace('"','\"',$single['incident_title'])."</a>")) . "\",";
