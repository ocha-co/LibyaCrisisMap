...
class Incident_Model extends ORM
{
	...
	*/
	...
	function getIncidents($filter, $loadCategories = false){
		...
		if($loadCategories){
				if(count($ids)){
				$categories = ORM::factory('incident_category')->where("incident_id in (".implode(",", $ids).")")->find_all();
	...
	function getFormData($filter){
		$sql           = 'select id from incident where '.$filter;
		$formResponses = ORM::factory('form_response')->where('incident_id in ('.$sql.')')->find_all();
		$responses     = array();
		
		foreach($formResponses as $response){
			$responses[$response->incident_id][$response->form_field_id] = $response->form_response;
		}
		return $responses;
	}
}
