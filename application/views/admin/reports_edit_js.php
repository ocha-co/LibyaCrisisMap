...
?>
		...
		$(document).ready(function() {
			...
			$('.btn_save_close').live('click', function () {
				 var sc = false; // is any source category checked?
			    $('li > input[type=checkbox]', $("input[type=checkbox][value=20]").siblings("ul")).each(function(){ if($(this).attr('checked')) sc = true; });
			    if(sc)
			      $(this).parents("form").submit();
			    else
			      alert("Please confirm that you select the apropiate source category");
			    return false;
			});
